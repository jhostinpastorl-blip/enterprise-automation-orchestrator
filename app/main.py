...
def prometheus_metrics() -> str:
    ...
    lines.extend(
        [
            "# HELP automation_queue_depth Requests waiting or scheduled for retry.",
            "# TYPE automation_queue_depth gauge",
            f"automation_queue_depth {snapshot.queue_depth}",
            "# HELP automation_retrying_count Requests currently waiting for retry.",
            "# TYPE automation_retrying_count gauge",
            f"automation_retrying_count {snapshot.retrying_count}",
            "# HELP automation_dead_letter_count Requests currently in dead letter.",
            "# TYPE automation_dead_letter_count gauge",
            f"automation_dead_letter_count {snapshot.dead_letter_count}",
            "# HELP automation_oldest_queued_age_seconds Age of oldest queued or retrying request.",
            "# TYPE automation_oldest_queued_age_seconds gauge",
        ]
    )
    if snapshot.oldest_queued_age_seconds is not None:
        lines.append(
            f"automation_oldest_queued_age_seconds {snapshot.oldest_queued_age_seconds:.3f}"
        )
    return "\n".join(lines) + "\n" + render_http_metrics()
