...
class AutomationWorker:
    def __init__(
        ...
        self.retry_delay_seconds = int(os.getenv("RETRY_DELAY_SECONDS", "5")) if retry_delay_seconds is None else retry_delay_seconds
    ...
    def _backoff(self, attempt: int) -> int:
        base = min(self.max_retry_delay_seconds, self.retry_delay_seconds * (2 ** max(0, attempt - 1)))
    ...
    def run_once(self) -> bool:
        ...
        if cooldown_until > time.monotonic():
            ...
            logger.warning("target_backpressure request_id=%s target=%s retry_after_seconds=%s", request_id, request.target, delay)
            self.repository.fail_or_retry(request_id, f"Target '{request.target}' is cooling down", delay)
            return True

        adapter = ADAPTERS[request.execution_channel]
        logger.info("automation_execution_started request_id=%s process=%s target=%s channel=%s", request_id, request.process, request.target, request.execution_channel.value)
        try:
            detail = adapter.execute(request)
        except RetryableAdapterError as exc:
            attempt = self.repository.get(request_id).attempt_count
            retry_delay = exc.retry_after_seconds if exc.retry_after_seconds is not None else self._backoff(attempt)
            if exc.retry_after_seconds is not None:
                self._target_cooldowns[request.target] = time.monotonic() + retry_delay
            logger.warning("automation_execution_rate_limited request_id=%s target=%s retry_after_seconds=%s", request_id, request.target, retry_delay)
            self.repository.fail_or_retry(request_id, detail=str(exc), retry_delay_seconds=retry_delay)
        except Exception as exc:
            attempt = self.repository.get(request_id).attempt_count
            retry_delay = self._backoff(attempt)
            logger.exception("automation_execution_failed request_id=%s retry_after_seconds=%s", request_id, retry_delay)
            self.repository.fail_or_retry(request_id, detail=str(exc), retry_delay_seconds=retry_delay)
        else:
...
def main() -> None:
    ...
    AutomationWorker().run_forever(poll_interval_seconds=float(os.getenv("WORKER_POLL_INTERVAL_SECONDS", "1")))
